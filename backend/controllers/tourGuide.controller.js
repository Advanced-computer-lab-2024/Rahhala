...
export const createAccoutRequest = async (req, res) => {
  const { username, idCardImage, certificationImages, email, password, mobileNumber, yearsOfExperience, previousWork } = req.body;
  ...
  try {
    ...
    res.status(201).json({ message: "Tour guide account created successfully", profile: user });
  } catch (error) {
